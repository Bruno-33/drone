...
FTC_IMU imu; 
Vector3f old_v(0,0,0) ;
Vector3f atti(0,0,4096),atti_yaw(4096,0,0);
...
{/*
	...
	*/
	...
	DCM_angle = (gyro + old_v)* deltaT * 0.5;
	old_v = gyro ;
	
	dcm.from_euler(DCM_angle) ; 
	atti=CF_1st(dcm * atti,acc,ftc.factor.gyro_cf);
	atti.get_rollpitch(angle);
	
	atti_yaw=CF_1st(dcm * atti_yaw,acc,ftc.factor.gyro_cf);
	atti_yaw.get_yaw(angle);
	//angle.z += degrees(atti.z) ;
}
...
{
	static Vector3f err_I(0,0,0) ;
	acc.normalize() ;
	Vector3f g_vec;
	
	Q.vector_gravity(g_vec) ;
